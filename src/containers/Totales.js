...
import TotalesPage from './TotalesPage';
...
_onChangeIni(date) {
 console.log(date);
    this.setState({
      fechaini: date,
      btnini: moment(date).format('YYYY-MM-DD')
    })
}
_onChangeEnd(date) {
...
_ApplyRange() {
    ...
    const URL = 'http://tweetscron.genio.soy/totales.php';
    let postu = " " + this.state.post;
    ...
    <select ref="interval" className="visible"  onChange={this._onChangeInterval.bind(this)}>
      ...
      <option value="">Intervalo</option>
        ...
        <a href="javascript:void()"  onClick={this._closeI.bind(this)}>X</a> </div></div></div>
          ...
          <TotalesPage title="Totales" data={data}  searchKey={this.state.searchKey} post={this.state.post}/>
          </div>
      </div>
...
export default withWidth()(Totales);